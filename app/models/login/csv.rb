require 'csv'
class Login::Csv < ApplicationRecord

  attr_accessor :file

  has_many :csv_lines, ->{order(:id) }, :dependent => :destroy
  has_many :valid_csv_lines, ->{where(data_invalid: 0).order(:id) }, :class_name => 'Login::CsvLine'
  has_many :invalid_csv_lines, ->{where(data_invalid: 1).order(:id) }, :class_name => 'Login::CsvLine'
